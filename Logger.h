...
public:
	...
	void log(LogType type, boost::format fmt)
	{
		logClass(type, 0, boost::str(fmt));
	}
	void logError(boost::format fmt)
	{
		logClass(Error, 0, boost::str(fmt));
	}
	void logWarning(boost::format fmt)
	{
		logClass(Warning, 0, boost::str(fmt));
	}
	void logInfo(boost::format fmt)
	{
		logClass(Info, 0, boost::str(fmt));
	}
#endif
	...
	{
		logClass(Error, 0, msg);
	}
	void logWarning(const std::string& msg)
	{
		logClass(Warning, 0, msg);
	}
...
public:
	...
	void disableClass(int msgClass)
	{
		m_disabledClasses.push_back(msgClass);
	}
